...
namespace KeePassNatMsg
{
    public sealed class KeePassNatMsgExt : Plugin, IDisposable
    {
        ...
        /// <summary>
        /// an arbitrarily generated uuid for the keepassnatmsg generated password group
        /// </summary>
        public static readonly byte[] KeePassNatMsgGeneratedPasswordsGroupUuid = {
                0x5b, 0x26, 0x8c, 0x47, 0x3d, 0x7a, 0x4d, 0x9a,
                0x91, 0x40, 0x1d, 0x56, 0xb8, 0x2f, 0xc3, 0x71
        };
        ...
        public const string KeePassNatMsgGeneratedPasswordsGroupName = "KeePassNatMsg Generated Passwords";
        ...
        internal PwGroup GetGeneratedPasswordsGroup()
        {
            var root = GetConnectionDatabase().RootGroup;
            var uuid = new PwUuid(KeePassNatMsgGeneratedPasswordsGroupUuid);
            var group = root.FindGroup(uuid, true);
            if (group == null)
            {
                group = new PwGroup(false, true, KeePassNatMsgGeneratedPasswordsGroupName, PwIcon.Key);
                group.Uuid = uuid;
                root.AddGroup(group, true);
                UpdateUI(null);
            }
        ...
        internal string GeneratePassword()
        {
            ...
            PwProfile selectedProfile = GetConfiguredPasswordProfile();
            PwGenerator.Generate(out psNew, selectedProfile, pbEntropy, Program.PwGeneratorPool);

            byte[] pbNew = psNew.ReadUtf8();
            if (pbNew != null)
            {
                var password = StrUtil.Utf8.GetString(pbNew);
                MemUtil.ZeroByteArray(pbNew);
                SaveGeneratedPasswordBackup(password);
                return password;
            }
            return null;
        }

        private void SaveGeneratedPasswordBackup(string password)
        {
            ...
            {
                var db = GetConnectionDatabase();
                var group = GetGeneratedPasswordsGroup();
                var entry = new PwEntry(true, true);
                var timestamp = DateTime.Now.ToString("yyyy/M/d H:mm:ss");

                entry.Strings.Set(PwDefs.TitleField,
                    new ProtectedString(false, "KeePassNatMsg generated password at: " + timestamp));
                ...
                entry.Strings.Set(PwDefs.NotesField,
                    new ProtectedString(false,
                        "Generated by KeePassNatMsg from the browser password generator."));
            ...
            {
                ShowNotification("Generated password backup could not be saved: " + ex.Message);
            }
        }

        private void AutoSaveIfRequired(PwDatabase db)
        {
            if (!Program.Config.Application.AutoSaveAfterEntryEdit) return;

            HostInstance.MainWindow.Invoke(new MethodInvoker(() =>
            {
                HostInstance.MainWindow.SaveDatabase(db, null);
            }));
        }

        internal PwProfile GetConfiguredPasswordProfile()
        {
            var passwordGenerator = Program.Config.PasswordGenerator;
            var profileName = new ConfigOpt(HostInstance.CustomConfig).PasswordGeneratorProfileName;

            if (!string.IsNullOrWhiteSpace(profileName))
            {
                var selectedProfile = KeePass.Util.PwGeneratorUtil.GetAllProfiles(true)
            ...
            return passwordGenerator.AutoGeneratedPasswordsProfile;
        }
